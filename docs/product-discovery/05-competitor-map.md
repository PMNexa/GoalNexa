...
|---|---|---|---|---|
| **[Operately](https://github.com/operately/operately)** | Teams of 5-100 (ops teams, nonprofits, regulated/compliance-focused teams, startups) | "The open source company operating system" — goals/OKRs + projects + structured check-ins, "self-hostable and built for teams that prefer transparency over black-box SaaS" | Custom/"Other" license (open-core pattern); self-hosted **free, unlimited users**; cloud tiers $0 (≤10 users) / $49 (≤30) / $149 (≤100) / $249 (unlimited) per month, flat-rate not per-seat; one-click deploy via Railway | **559 GitHub stars, 70 forks, 1,003 total issues, actively committed (pushed within the last day as of 2026-09-20)** — by far the most mature genuinely self-hostable team-goal product found in this whole research effort, ~3x BurningOKR's star count. This is a materially stronger data point than the rest of this table and should be weighted accordingly. |
...
**Revised 2026-09-20 — the "no mature self-hosted OKR alternative exists" claim was wrong.** [Operately](https://github.com/operately/operately) is a real, actively-maintained counterexample (559 stars, 1,003 issues, committed within the last day, one-click Railway deploy, free-unlimited-self-host + flat-rate paid cloud). Its existence and traction is itself a demand signal this research missed on the first two passes — a product matching almost exactly the "free and easy to access" self-hosted team-goals wedge already exists and is growing, not merely surviving. This doesn't mean the space is saturated (559 stars is still small next to Twenty CRM's ~50K), but it does mean "nobody has built this and it's unproven" is no longer an accurate description of the landscape.

1. **The gap is narrower than originally framed, not absent.** No mainstream *SaaS* OKR vendor self-hosts, and most open-source attempts (Open OKR, okr2go, puzzle/okr) are pre-production or abandoned — but Operately shows a well-executed, actively-maintained entrant in this exact space can gain real traction. The wedge is "build it better/differently than Operately," not "build the first one."
